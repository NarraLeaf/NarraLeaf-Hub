...
<root>/hub.db                      the accounts, the invitations, the projects and the decisions
...
about a version rather than about one Hub, so it goes in a per-user cache, one
copy per version per machine:

```
%LOCALAPPDATA%\nlhub\cache\bin\    Windows
$XDG_CACHE_HOME/nlhub/bin/         Linux, or ~/.cache/nlhub/bin/
~/Library/Caches/nlhub/bin/        macOS
```
...
storage root meant a prompt and a permanent rule for every Hub anybody ever
started, including every throwaway one. `NLHUB_CACHE_DIR` names a directory to
...
A Hub that has already run keeps whatever it has. An installation under
...
started from, which Windows refuses outright. Deleting `<root>/bin` while Hub is
...
same cache directory. They are Epic Games' terms for the program Hub installs on
...
```sh
nlhub settings list --root /srv/hub
nlhub settings set token.sign_in_lifetime_seconds 7d --root /srv/hub
nlhub settings set token.repository_lifetime_seconds 5m --root /srv/hub
```
...
something somebody chose here, because those are different facts — a Hub that
never chose follows a later version of Hub if the default moves, and one that
...
Hub's log with the caller, the resource and the outcome, and kept in `hub.db`
...
age: a Hub used twice a month would have its whole history deleted between
...
question is not a Hub waiting on a platter.
